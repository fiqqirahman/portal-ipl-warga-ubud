...
class JabatanVendorDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return datatables()
            ...
            ->addColumn('aksi', function ($row) {
                $routeEdit = route('master.jabatan-vendor.edit', enkrip($row->id));
                $button = '<div class="d-flex justify-content-start">';
                $routeUpdateStatus = route($row->status_data == 1 ? 'master.jabatan-vendor.nonaktif' : 'master.jabatan-vendor.aktif', enkrip($row->id));
                ...
                $button .= '</div>';
                if (!Gate::allows(PermissionEnum::MasterJabatanVendorEdit->value)) {
                    $button = '-';
                }
                return $button;
            })
     ...
     */
    public function query(\App\Models\Master\JabatanVendor $model): QueryBuilder
    {
    ...
    {
        return $this->builder()
            ->setTableId('jabatanvendor-table')
    ...
    {
        return 'JabatanVendor_' . date('YmdHis');
    }
}
