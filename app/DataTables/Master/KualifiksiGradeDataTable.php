...
use App\Models\KualifiksiGrade;
...
class KualifiksiGradeDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return datatables()
            ...
            ->addColumn('aksi', function ($row) {
                $routeEdit = route('master.kualifikasi-grade.edit', enkrip($row->id));
                $button = '<div class="d-flex justify-content-start">';
                $routeUpdateStatus = route($row->status_data == 1 ? 'master.kualifikasi-grade.nonaktif' : 'master.kualifikasi-grade.aktif', enkrip($row->id));
                ...
                $button .= '</div>';
                if (!Gate::allows(PermissionEnum::MasterKualifikasiGradeEdit->value)) {
                    $button = '-';
                }
                return $button;
            })
     ...
     */
    public function query(KualifikasiGrade $model): QueryBuilder
    {
    ...
    {
        return 'KualifiksiGrade_' . date('YmdHis');
    }
}
