...
class KategoriVendorDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return datatables()
            ...
            ->addColumn('aksi', function ($row) {
                $routeEdit = route('master.kategori-vendor.edit', enkrip($row->id));
                $button = '<div class="d-flex justify-content-start">';
                $routeUpdateStatus = route($row->status_data == 1 ? 'master.kategori-vendor.nonaktif' : 'master.kategori-vendor.aktif', enkrip($row->id));
                ...
                $button .= '</div>';
                if (!Gate::allows('master_kategori_vendor_edit')) {
                    $button = '-';
                }
                return $button;
            })
     ...
     */
    public function query(\App\Models\Master\KategoriVendor $model): QueryBuilder
    {
    ...
    {
        return 'KategoriVendor_' . date('YmdHis');
    }
}
