...
class SubBidangUsahaDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return datatables()
            ...
            ->addColumn('aksi', function ($row) {
                $routeEdit = route('master.sub-bidang-usaha.edit', enkrip($row->id));
                $button = '<div class="d-flex justify-content-start">';
                $routeUpdateStatus = route($row->status_data == 1 ? 'master.sub-bidang-usaha.nonaktif' : 'master.sub-bidang-usaha.aktif', enkrip($row->id));
                ...
                $button .= '</div>';
                if (!Gate::allows(PermissionEnum::MasterSubBidangUsahaEdit->value)) {
                    $button = '-';
                }
                return $button;
            })
     ...
     */
    public function query(\App\Models\Master\SubBidangUsaha $model): QueryBuilder
    {
    ...
    {
        return 'SubBidangUsaha_' . date('YmdHis');
    }
}
