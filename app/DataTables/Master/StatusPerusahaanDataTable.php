...
class StatusPerusahaanDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return datatables()
            ...
            ->addColumn('aksi', function ($row) {
                $routeEdit = route('master.status-perusahaan.edit', enkrip($row->id));
                $button = '<div class="d-flex justify-content-start">';
                $routeUpdateStatus = route($row->status_data == 1 ? 'master.status-perusahaan.nonaktif' : 'master.status-perusahaan.aktif', enkrip($row->id));
                ...
                $button .= '</div>';
                if (!Gate::allows('master_status_perusahaan_edit')) {
                    $button = '-';
                }
                return $button;
            })
     ...
     */
    public function query(\App\Models\Master\StatusPerusahaan $model): QueryBuilder
    {
    ...
    {
        return 'StatusPerusahaan_' . date('YmdHis');
    }
}
