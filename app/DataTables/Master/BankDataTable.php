...
class BankDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return datatables()
            ...
            ->addColumn('aksi', function ($row) {
                $routeEdit = route('master.bank.edit', enkrip($row->id));
                $button = '<div class="d-flex justify-content-start">';
                $routeUpdateStatus = route($row->status_data == 1 ? 'master.bank.nonaktif' : 'master.bank.aktif', enkrip($row->id));
                ...
                $button .= '</div>';
                if (!Gate::allows(PermissionEnum::MasterBankEdit->value)) {
                    $button = '-';
                }
                return $button;
            })
     ...
     */
    public function query(\App\Models\Master\Bank $model): QueryBuilder
    {
