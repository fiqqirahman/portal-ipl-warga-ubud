...
class JenisIdentitasDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return datatables()
            ...
            ->addColumn('aksi', function ($row) {
                $routeEdit = route('master.jenis-identitas.edit', enkrip($row->id));
                $button = '<div class="d-flex justify-content-start">';
                $routeUpdateStatus = route($row->status_data == 1 ? 'master.jenis-identitas.nonaktif' : 'master.jenis-identitas.aktif', enkrip($row->id));
                ...
                $button .= '</div>';
                if (!Gate::allows(PermissionEnum::MasterJenisIdentitasEdit->value)) {
                    $button = '-';
                }
                return $button;
            })
     ...
     */
    public function query(JenisIdentitas $model): QueryBuilder
    {
    ...
    {
        return $this->builder()
            ->setTableId('jenisidentitas-table')
    ...
    {
        return 'JenisIdentitas_' . date('YmdHis');
    }
}
