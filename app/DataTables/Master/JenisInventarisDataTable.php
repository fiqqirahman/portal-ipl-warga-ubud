...
class JenisInventarisDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return datatables()
            ...
            ->addColumn('aksi', function ($row) {
                $routeEdit = route('master.jenis-invetaris.edit', enkrip($row->id));
                $button = '<div class="d-flex justify-content-start">';
                $routeUpdateStatus = route($row->status_data == 1 ? 'master.jenis-invetaris.nonaktif' : 'master.jenis-invetaris.aktif', enkrip($row->id));
                ...
                $button .= '</div>';
                if (!Gate::allows(PermissionEnum::MasterJenisInventarisEdit->value)) {
                    $button = '-';
                }
                return $button;
            })
     ...
     */
    public function query(JenisInventaris $model): QueryBuilder
    {
    ...
    {
        return $this->builder()
            ->setTableId('jenisinventaris-table')
    ...
    {
        return 'JenisInventaris_' . date('YmdHis');
    }
}
